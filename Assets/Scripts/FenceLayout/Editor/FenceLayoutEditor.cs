...
public class FenceLayoutEditor : Editor
{
	...
	public void OnEnable()
	{
		m_fenceLayout = target as FenceLayout;
			...
			// Unity 2020.1+ permite incluir inactivos
			m_terrains = FindObjectsOfType<Terrain>(includeInactive: true);
	#else
			// Versiones anteriores usan el método sin parámetros
			m_terrains = FindObjectsOfType<Terrain>();
	#endif

	#if UNITY_2019_1_OR_NEWER
			// Nuevo callback de SceneView desde 2019.1
			SceneView.duringSceneGui += InputUpdate;
	#else
			// API antigua para versiones anteriores a 2019.1
